...
function _getField(context, ast, parentType) {
  ...
  var variableValues = context.variableValues; // Current schema type
  ...
  typeName = targetType.astNode && targetType.astNode.kind === 'InterfaceTypeDefinition' ? '' : typeName; // Child query params

  var args = ast.arguments.reduce(function (obj, argument) {
...
function getFieldSelectionSet(context) {
  ...
  parentType = parentType || context.schema.getType(node); // for recursion: fragments doesn't have many sets
  ...
  return selections.reduce(function (set, ast) {
    ...
    switch (ast.kind) {
      case 'Field':
        ...
        return _extends({}, set, _defineProperty({}, ast.name.value, _getField(context, ast, parentType)));
